...
addUser(name: string, email: string, password: string, password_confirm: string, usertype: string) {
  ...
  .subscribe(
   ...
   error => {console.log(error)
    this.notifier.notify('error', 'Nie udało się dodać nowego użytkownika');
  });
}
updateUser(name: string, email: string, usertype: string, id: number) {
  ...
  .subscribe( succ => {console.log(succ)
    this.notifier.notify('success', 'Pomyślnie edytowano użytkownika');
  }, 
  error => {console.log(error)
    this.notifier.notify('error', 'Nie udało się edytować użytkownika');
  });
}
deleteUser(id: number){
  ...
  error => {console.log(error)
    this.notifier.notify('success', 'Nie udało się usunąć użytkownika');
  });
}
